...
function JobCardProcess({ navigation, route }) {
  ...
  const isCutAndSeal = currentRoute?.Process?.name?.toLowerCase()?.includes("cut & seal") ?? false;
 ...
 const processqty = useMemo(() => {
  ...
  const isProcessStarted = pauseable || resumable || !!punchId || !!punch_data?.id;
  ...
  useEffect(() => {
    ...
    if (isCutAndSeal) {
      totalCompleted = Object.values(splitQty).reduce((acc, val) => acc + Number(val || 0), 0);
    } else {
  ...
  try {
   ...
   async function stopProcess() {
     ...
     try {
      ...
      if (isCutAndSeal) {
        const hasAnyQty = Object.values(splitQty).some(val => Number(val) > 0);
        if (!hasAnyQty) {
          return Alert?.alert("Missing", "Please enter at least one valid quantity for split sizes!");
        }
      } else {
    ...
    if (isCutAndSeal) {
      ...
      submitBody.completedQty = Object.values(splitQty).reduce((acc, val) => acc + Number(val || 0), 0);
    }
...

{
   isnotLabel && (
     <>
        ...
        <InfoRow label="Running Qty" c={c} spacing={spacing} styles={styles}>
          <AppText style={{ color: c.textMuted }}>
            {jobcard?.runningQty ?? '—'}
          </AppText>
        </InfoRow>  
      ...
      <InfoRow label="Gsm & Size" c={c} spacing={spacing} styles={styles}>
          <AppText style={{ color: c.textMuted }}>
            {(() => {
              ...
              const processBqList = bqList.filter(b => 
                b?.processId === currentRoute?.Process?.id || 
                b?.Process?.id === currentRoute?.Process?.id
              );
              
              const listToUse = processBqList.length > 0 ? processBqList : bqList;

              return listToUse.map(b => {
                const sizeName = b?.FullBoardSize?.name || b?.FullBoardSize || '—';
...
{
  isnotLabel && (  <>
         <InfoRow label="No. of Sheets" c={c} spacing={spacing} styles={styles}>
          <AppText style={{ color: c.textMuted }}>
            {(() => {
              const bqList = jobcard?.boardQualities || [];
              const processBqList = bqList.filter(b => 
...
{  isnotLabel  ?  (  <>
      ...
      <View style={{ flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'flex-start', paddingHorizontal: spacing.md, marginTop: spacing.lg, width: '100%' }}>
        ...
        <View style={{ width: '48%', marginBottom: 0 }}>
          {isCutAndSeal && jobcard?.jobCardSizeDetails?.length > 0 ? (
            <View>
              <AppText variant="sm" muted style={{ marginBottom: spacing.xs, marginLeft: spacing.xs }}>
                Split Sizes Qty
               </AppText>
               <TouchableOpacity 
                 ...
                 <AppText 
                   variant="sm" 
                   style={{ 
                     color: Object.keys(splitQty).length > 0 && Object.values(splitQty).some(val => Number(val) > 0) ? c.text : c.textMuted,
                     marginLeft: spacing.xs 
                   }}
                   numberOfLines={1}
                 >
                   {Object.keys(splitQty).length > 0 && Object.values(splitQty).some(val => Number(val) > 0)
                     ? Object.entries(splitQty)
                         .filter(([_, qty]) => Number(qty) > 0)
                         .map(([sId, qty]) => {
                           const sizeObj = jobcard?.jobCardSizeDetails?.find(s => String(s.id) === String(sId));
                           const sizeName = sizeObj?.Size?.name || sizeObj?.sizeId || 'Size';
                           return `${sizeName} (${qty})`;
                         }).join(', ')
                     : 'Sizes'}
                 </AppText>
  ...
  closeOnBackdrop={true} >
         <View style={{ paddingTop: spacing.sm, paddingBottom: spacing.lg }}>
             {jobcard?.jobCardSizeDetails?.map((sizeObj, idx) => (
                <View key={sizeObj?.id || idx} style={{ flexDirection: 'row', alignItems: 'center', marginBottom: spacing.md }}>
                  <AppText style={{ flex: 1, color: c.textMuted, fontSize: typography.sm.fontSize, fontWeight: '500' }}>
                    {sizeObj?.Size?.name || sizeObj?.sizeId || `Split ${idx + 1}`} ({sizeObj?.qty ?? 0})
                  </AppText>
                  <View style={{ flex: 1.2 }}>
                    <AppInput
                      placeholder="Qty"
      ...
      >
        <View style={{ paddingTop: spacing.sm, paddingBottom: spacing.lg }}>
           ...
           {(pauseReason === "Partially Completed" || pauseReason === "Others") && !isCutAndSeal && (
             <View style={{ marginTop: spacing.md }}>
               <AppText style={{ marginBottom: spacing.xs, fontSize: typography.sm.fontSize, color: c.textMuted }}>
           ...
           {pauseReason === "Partially Completed" && isCutAndSeal && (
             <View style={{ marginTop: spacing.md }}>
               ...
               {jobcard?.jobCardSizeDetails?.map((sizeObj, idx) => (
                 <View key={sizeObj?.id || idx} style={{ flexDirection: 'row', alignItems: 'center', marginBottom: spacing.sm }}>
                   <AppText style={{ flex: 1, color: c.textMuted, fontSize: typography.sm.fontSize, fontWeight: '500' }}>
                     {sizeObj?.Size?.name || sizeObj?.sizeId || `Split ${idx + 1}`} ({sizeObj?.qty ?? 0})
                   </AppText>
                   <View style={{ flex: 1.2 }}>
           ...
           <View style={{ marginTop: spacing.lg, alignItems: 'center' }}>
             <AppButton
               ...
               onPress={() => {
                 if (!pauseReason) {
                   return Alert?.alert("Required", "Please select a reason for pausing.");
                 }
                 if (pauseReason === "Partially Completed") {
                   if (isCutAndSeal) {
                     const totalSplitQty = Object.values(splitQty).reduce((acc, val) => acc + Number(val || 0), 0);
                     if (totalSplitQty <= 0) {
                        return Alert?.alert("Required", "Please enter valid quantities for split sizes.");
                     }
                     if (totalSplitQty > Number(processqty)) {
                        return Alert?.alert("Invalid", "Total quantity cannot exceed Production Qty.");
                     }
                   } else {
                     if (!pauseQty || Number(pauseQty) <= 0) {
                       return Alert?.alert("Required", "Please enter a valid quantity.");
                     }
                     if (Number(pauseQty) > Number(processqty)) {
                       return Alert?.alert("Invalid", "Quantity cannot exceed Production Qty.");
                     }
                   }
                 }
                 if (pauseReason === "Others" && (!pauseRemarks || !pauseRemarks.trim())) {
                   return Alert?.alert("Required", "Please enter remarks for 'Others'.");
                 }
                 
                 const finalReason = pauseReason === "Others" ? `Others - ${pauseRemarks.trim()}` : pauseReason;
                 PauseProcess(finalReason, (pauseReason === "Partially Completed" && !isCutAndSeal) ? pauseQty : null);
